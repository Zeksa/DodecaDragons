...
function populateTabPositions() {
  ...
  tabNames.forEach((tab) => {
    highestUnlock = Math.max(highestUnlock, tabData[tab][2]);
  });
...
function render(x, y) {
  ...
  if (game.unlocks >= 3) {
    ...
    if (game.unlocks >= 6) {
      cachedBoxes[7].style.top = y - 130 + "px";
    } else {
      cachedBoxes[7].style.top = y - 235 + "px";
    }
  }
  if (game.unlocks >= 4) {
  ...
  if (game.unlocks >= 29)
    cachedBoxes[38].style.backgroundPosition = x / 6 + "px " + y / 6 + "px";
...
function renderKeyboardPan() {
  ...
  if (
    ...
    (Math.abs(inputVars.keySpeedX) < 0.1 &&
      Math.abs(inputVars.keySpeedY) < 0.1) ||
    bigFinishPoint > 0
  )
    return;
...
function mouseDown(e) {
  if (renderVars.isAutoPanning || e.button !== 0) {
    return;
  } //ensure we only respond to left clicks
...
async function panTo(endX, endY) {
  ...
  while (Date.now() < midTime) {
    renderVars.posX = lerp(
      startX,
      midX,
      ((Date.now() - startTime) / renderVars.autoPanTime) * 1.33
    );
    renderVars.posY = lerp(
      startY,
      midY,
      ((Date.now() - startTime) / renderVars.autoPanTime) * 1.33
    );
  ...
  while (Date.now() < endTime) {
    renderVars.posX = lerp(
      midX,
      endX,
      ((Date.now() - midTime) / renderVars.autoPanTime) * 4
    );
    renderVars.posY = lerp(
      midY,
      endY,
...
function panToTab(tab) {
  if (tabData[tab] === undefined) {
    console.warn("auto tab pan attempted with invalid parameter: " + tab);
    return;
  }
  panTo(
    -tabData[tab][0] + window.innerWidth / 2,
...
function handleMouseMove(event) {
  ...
  if (
    ...
    if (
      Date.now() - renderVars.lastRender >= 20 &&
  ...
  if (game.nuclearPastaStateInfo)
    document.getElementById("nuclearPastaInfoWindow").style.left =
      renderVars.mousePosX + 4 + "px";
  if (game.nuclearPastaStateInfo)
    document.getElementById("nuclearPastaInfoWindow").style.top =
      renderVars.mousePosY + 4 + "px";
}
...
function updatePanKeys() {
  if (document.querySelector("#dragonNameBox") != document.activeElement) {
    if (inputVars.isMobile) {
      inputVars.keyInputX =
        inputVars.keysHeld["a"] +
        inputVars.keysHeld["ArrowLeft"] -
        inputVars.keysHeld["d"] -
        inputVars.keysHeld["ArrowRight"];
      inputVars.keyInputY =
        inputVars.keysHeld["w"] +
        inputVars.keysHeld["ArrowUp"] -
        inputVars.keysHeld["s"] -
        inputVars.keysHeld["ArrowDown"];
    } else {
      inputVars.keyInputX =
        inputVars.keysHeld["a"] +
        inputVars.keysHeld["ArrowLeft"] -
...
function updatePanKeySpeed() {
  ...
  if (Math.sign(inputVars.keySpeedX) * Math.sign(inputVars.keyInputX) === -1)
    inputVars.keySpeedX = 0; // skip decel if opposite direction is pressed
  if (Math.sign(inputVars.keySpeedY) * Math.sign(inputVars.keyInputY) === -1)
    inputVars.keySpeedY = 0;
  if (inputVars.keyInputX === 0) {
    inputVars.keySpeedX -= Math.sign(inputVars.keySpeedX) * effectiveDecel;
    if (Math.abs(inputVars.keySpeedX) <= effectiveDecel)
      inputVars.keySpeedX = 0;
  } else {
    inputVars.keySpeedX =
      Math.sign(inputVars.keyInputX) * inputVars.keySpeedCap; // no accel, goes straight to max speed
  }
  if (inputVars.keyInputY === 0) {
    inputVars.keySpeedY -= Math.sign(inputVars.keySpeedY) * effectiveDecel;
    if (Math.abs(inputVars.keySpeedY) <= effectiveDecel)
      inputVars.keySpeedY = 0;
  } else {
    inputVars.keySpeedY =
      Math.sign(inputVars.keyInputY) * inputVars.keySpeedCap;
  }
  inputVars.keySpeedX =
    Math.round(inputVars.keySpeedX * inputVars.keyboardRenderPerSec) /
    inputVars.keyboardRenderPerSec; // avoiding precision errors (hopefully)
  inputVars.keySpeedX = Math.min(
    inputVars.keySpeedCap,
    Math.max(-inputVars.keySpeedCap, inputVars.keySpeedX)
  ); // clamp between the speed caps
  inputVars.keySpeedY =
    Math.round(inputVars.keySpeedY * inputVars.keyboardRenderPerSec) /
    inputVars.keyboardRenderPerSec;
  inputVars.keySpeedY = Math.min(
    inputVars.keySpeedCap,
    Math.max(-inputVars.keySpeedCap, inputVars.keySpeedY)
  );
}
...
function touchDown(event) {
  if (
    renderVars.isAutoPanning ||
    !event.changedTouches ||
    !event.changedTouches[0]
  )
    return; //make sure the event data is proper
...
arrowElements.up.addEventListener("pointerdown", (e) => {
  arrowClick("ArrowUp");
});
arrowElements.up.addEventListener("pointerup", (e) => {
  arrowRelease("ArrowUp");
});

arrowElements.down.addEventListener("pointerdown", (e) => {
  arrowClick("ArrowDown");
});
arrowElements.down.addEventListener("pointerup", (e) => {
  arrowRelease("ArrowDown");
});

arrowElements.left.addEventListener("pointerdown", (e) => {
  arrowClick("ArrowLeft");
});
arrowElements.left.addEventListener("pointerup", (e) => {
  arrowRelease("ArrowLeft");
});

arrowElements.right.addEventListener("pointerdown", (e) => {
  arrowClick("ArrowRight");
});
arrowElements.right.addEventListener("pointerup", (e) => {
  arrowRelease("ArrowRight");
});

if (inputVars.isMobile) {
  //event for mobile only
  document.addEventListener("pointerup", (e) => {
    resetPressedKeys();
  }); //on mobile always reset all when any arrow is released, just to proof against weird edge cases
