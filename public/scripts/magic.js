...
function magicCheck() {
  if (game.magicToGet.eq(0)) {
    if (confirm(trads.magic.noGain)) magicReset();
  } else if (game.confirmations[0]) {
    if (confirm(trads.magic.confirmReset)) magicReset();
  } else {
    magicReset();
  }
}
...
function magicReset(triggerLayer = "magic") {
  ...
  if (game.unlockedAchievements[0] > 8) {
    game.miners = new Decimal(1);
  } else {
  ...
  if (
    (triggerLayer === "magic" && game.unlockedAchievements[3] < 5) ||
    (triggerLayer === "sigil" && game.unlockedAchievements[7] < 3) ||
  ...
  if (game.magic.gt(magicHardcap)) {
    ...
    document.getElementById("magicCap").innerHTML = "";
  }
}
...
function buyMagicUpgrade(x) {
  ...
  if (
    ...
    game.magic.gte(magicUpgradeCosts[x - 1])
  ) {
    ...
    if (x == 8)
      document.getElementsByClassName("platinumUpgrade")[6].style.display =
        "block";
    if (x == 11 && game.platinumUpgradesBought[6] == 10)
    ...
    if (x == 12) {
      ...
      document.getElementById("moreMagicUpgradesButton").style.display =
        "block";
      addUnlock(); //sets unlock to 5
    }
    if (x == 14)
      document.getElementById("fireUpgrade1Effect").textContent = format(
        new Decimal(3.5).pow(game.fireUpgrade1Bought.pow(0.7)),
        2
      );
    if (x == 19)
    ...
    if (x == 20) {
      ...
      document.getElementsByClassName("resourceRow")[6].style.display =
        "inline-block";
...
function magicUpgradeBuyMax() {
  for (i = 1; i < 21; i++) buyMagicUpgrade(i);
}
...
function unlockMoreMagicUpgrades() {
  if (game.magifolds.gte(150000) && game.magic.gte(2e8)) {
    ...
    document.getElementsByClassName("box")[7].style.top =
      window.innerHeight / 2 + renderVars.posY + renderVars.diffY - 133 + "px";
    for (i = 12; i < 20; i++) {
...
function buyDarkMagicUpgrade(x) {
  ...
  if (
    ...
    game.magic.gte(darkMagicUpgradeCosts[x - 1])
  ) {
...
function darkMagicUpgradeBuyMax() {
  ...
  if (game.pinkSigilUpgradesBought[1].eq(1)) {
    for (i = 9; i < 21; i++) buyDarkMagicUpgrade(i);
  }
}
...
function buyVoidMagicUpgrade(x) {
  ...
  if (
    ...
    game.magic.gte(voidMagicUpgradeCosts[x - 1])
  ) {
    game.voidMagicUpgradesBought[x - 1] = true;
    ...
    if (x == 4) {
      ...
      document.getElementsByClassName("resourceRow")[23].style.display =
        "block";
      ...
      document.getElementsByClassName("holyFireBuyMaxButton")[5].style.display =
        "inline-block";
      game.holyFireUpgradeCosts[5] = new Decimal(holyFireUpgradeBase[6])
        .pow(game.holyFireUpgradesBought[5])
        .mul(holyFireUpgradeInitialCosts[6])
        .floor();
      document.getElementById("holyFireUpgrade6Cost").textContent = format(
        game.holyFireUpgradeCosts[5],
        0
      );
    } else if (x == 16) {
      document.getElementsByClassName("box")[39].style.display = "block";
      document.getElementsByClassName("resourceRow")[25].style.display =
        "block";
      ...
      document.getElementsByClassName("resourceRow")[26].style.display =
        "block";
...
function getMagicGain() {
  ...
  if (game.magicUpgradesBought[3]) {
    if (game.magicUpgradesBought[9]) {
      toGet = toGet.mul(game.magic.add(1).log2().mul(4).add(1));
    } else {
      toGet = toGet.mul(game.magic.add(1).log2().mul(1.5).add(1));
    }
  }
  ...
  if (game.darkMagicUpgradesBought[1])
    toGet = toGet.pow(game.uranium.add(1).log10().div(30).add(1));
  magicHardcap = new Decimal("e5000000");
  if (game.tomeUpgradesBought[8] == true)
    magicHardcap = magicHardcap.pow(
      game.blueFireUpgradesBought[3].pow(0.7).mul(5).add(1)
...
function getNextMagicAt() {
  ...
  if (game.magicUpgradesBought[3]) {
    if (game.magicUpgradesBought[9]) {
      nextMagic = nextMagic.div(game.magic.add(1).log2().mul(4).add(1));
    } else {
      nextMagic = nextMagic.div(game.magic.add(1).log2().mul(1.5).add(1));
    }
  }
