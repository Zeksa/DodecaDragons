...
achievementBox.style.boxShadow =
...
function showAchievement(x, y) {
  ...
  //  }
  //}
  if (Object.keys(achievementRewards).indexOf(thisId) > -1) {
    //check if achievement has a reward, in which case we want to highlight it.
    //newHTML += "<img src='img/starFilled.png' class='achievementStar' />"
    newHTML +=
      "<p style='margin-top: 11px; color: #6ff'><span style='font-size: 22px'>" +
      romanNumerals[x] +
      "</span><br>" +
      achievementResources[y].shortName +
      "</p>";
  } else {
    //newHTML += "<img src='img/starEmpty.png' class='achievementStar' />"
    newHTML +=
      "<p style='margin-top: 11px'><span style='font-size: 22px'>" +
      romanNumerals[x] +
      "</span><br>" +
      achievementResources[y].shortName +
      "</p>";
  ...
  document
    .getElementById("achievementsDiv")
    .appendChild(achievementBox.cloneNode(true));
...
function showAchievements(x) {
  ...
  let unlocks =
    achievementDisplayUnlocks[x] ||
    achievementDisplayUnlocks[achievementDisplayUnlocks.length - 1]; //if no data for selected x value, default to highest unlock value
...
function showAchievementInfo(x, y) {
  let newHtml = "";
  if (x === null) {
    //show default text if no hovered achievement is detected
    newHtml +=
      "<p style='font-size: 22px; line-height: 22px; margin: 4px; color: #bbb'>Achievement name</p>";
    newHtml +=
      "<p style='color: #bbb'>Mouse over an achievement for more info!</p>";
    document.getElementById("achievementInfo").innerHTML = newHtml;
    return;
  }
  newHtml +=
    "<p style='font-size: 22px; line-height: 22px; margin: 4px'>" +
    achievementNames[x][y] +
    "</p>";
  newHtml +=
    "<p>Obtain " +
    format(achievementRequirements[x][y], 0) +
    " " +
    achievementResources[x].name +
    "."; //example: Obtain 1e10 fire
  ...
  if (Object.keys(achievementRewards).indexOf(thisId) > -1) {
    //check whether the acheivementRewards object has an entry for this id
    newHtml +=
      "<p style='color: #8ff'>Reward: " + achievementRewards[thisId] + "</p>";
  }
  /*
  switch (thisId) {
    case "0-4":
...
function checkAchievements() {
  for (let i = 0; i < Object.keys(achievementResources).length; i++) {
    //achievementResourceInternals tells us the internal names of the resources e.g. cyanSigils
    if (game.unlockedAchievements[i] > 23) {
      ...
      */
      ...
      if (
        !document.getElementById("ach" + i + "x" + game.unlockedAchievements[i])
      )
        continue; //skip this resource if the next achievement isn't yet loaded into the DOM
      while (
        loops < 10 &&
        achievementRequirements[i][game.unlockedAchievements[i]] !==
          undefined &&
        new Decimal(game[achievementResources[i].internalName]).gte(
          achievementRequirements[i][game.unlockedAchievements[i]]
        )
      ) {
        if (
          !document.getElementById(
            "ach" + i + "x" + game.unlockedAchievements[i]
          )
        )
          break; //break this loop if next achievement isn't yet loaded into the DOM
        game.unlockedAchievements[i] += 1;
        achievementBoxOpen(i, game.unlockedAchievements[i] - 1);
        document
          .getElementById("ach" + i + "x" + (game.unlockedAchievements[i] - 1))
          .classList.add("achievementUnlocked");
...
function processAchievementRewards() {
  ...
  //gold rewards
  if (game.unlockedAchievements[0] > 4)
    document.getElementById("platinumMaxAllButton").style.display = "block";
  if (game.unlockedAchievements[0] > 8)
    document.getElementById("minerAutoBuyMaxButton").style.display =
      "inline-block";
  if (game.unlockedAchievements[0] > 10) {
    document.getElementById("magicUpgradeBuyMaxButton").style.display = "block";
    document.getElementById("darkMagicUpgradeBuyMaxButton").style.display =
      "block";
  }
  //magic rewards
  if (game.unlockedAchievements[3] > 0)
    for (i = 0; i < 6; i++)
      document.getElementsByClassName("fireBuyMaxButton")[i].style.display =
        "inline-block";
  if (game.unlockedAchievements[3] > 1)
    document.getElementById("fireMaxAllButton").style.display = "block";
  //maybe showing divs for auto magic gain readout?
  //uranium rewards
  if (game.unlockedAchievements[5] > 2)
    document.getElementById("uraniumMaxAllButton").style.display = "block";
  //cyan sigil rewards
  if (game.unlockedAchievements[6] > 1)
    document.getElementById("dragonFeedButton").innerHTML =
      '<b>Feed your dragon</b><br>Turns some of your score and magifolds into dragon food<br>Next feed requires <a id="dragonFeedCost">' +
      format(game.dragonFeedCost, 0) +
      "</a> magifolds";
  if (game.unlockedAchievements[6] > 2)
    document.getElementById("fireAutoMaxAllButton").style.display = "block";

  //blue sigil rewards
  if (game.unlockedAchievements[7] > 0)
    document.getElementById("dragonFeedButton").innerHTML =
      '<b>Feed your dragon</b><br>Next feed requires <a id="dragonFeedCost">' +
      format(game.dragonFeedCost, 0) +
      "</a> magifolds";
  //violet sigil rewards
  if (game.unlockedAchievements[9] > 0) {
    document.getElementById("dragonPetStuff").style.display = "block";
  }
  //pink sigil rewards
  if (game.unlockedAchievements[10] > 1) {
    document.getElementsByClassName("box")[20].style.display = "block";
  }
  //knowledge rewards
  if (game.unlockedAchievements[11] > 0) {
    document.getElementById("sigilResetAutomation").style.display = "block";
  }
  if (game.unlockedAchievements[11] > 7) {
    document.getElementById("knowledgeAutoMaxAllButton").style.display =
      "block";
  }
  //tome rewards
  if (game.unlockedAchievements[12] > 2) {
    document.getElementById("knowledgeMaxAllButton").style.display = "block";
  }
  //blue fire rewards
  if (game.unlockedAchievements[13] > 2)
    for (i = 0; i < 6; i++)
  ...
  if (game.unlockedAchievements[18] > 2)
    document.getElementById("sigilResetterCycleModeSpan").style.display =
      "block";
  //plutonium rewards
  if (game.unlockedAchievements[19] > 0)
    document.getElementById("plutoniumMaxAllButton").style.display = "block";
  if (game.unlockedAchievements[19] > 2)
    document.getElementById("tomeUpgradeBuyMaxButton").style.display = "block";
  //holy fire rewards
  if (game.unlockedAchievements[21] > 3)
    for (i = 0; i < 5; i++)
  ...
  //Planet rewards
  if (game.unlockedAchievements[23] > 5)
    document.getElementById("planetBuyMaxButton").style.display =
      "inline-block";
  if (game.unlockedAchievements[23] > 6)
    for (i = 0; i < 5; i++)
      document.getElementsByClassName("plagueBuyMaxButton")[i].style.display =
        "inline-block";
  //Hypergod rewards
  ...
  if (game.unlockedAchievements[24] > 7)
    document.getElementById("finalityEssenceMaxAllButton").style.display =
      "block";
}
...
function achievementBoxOpen(x, y) {
  ...
  setTimeout(function () {
    document.getElementsByClassName("box")[18].style.transition =
      "left 800ms ease-in-out";
    document.getElementsByClassName("box")[18].style.left = "5px";
  }, 50);
  document.getElementById("achievementBoxInfo").innerHTML =
    "<b>" +
    achievementNames[x][y] +
    "</b> - Obtain " +
    format(achievementRequirements[x][y], 0) +
  ...
  if (Object.keys(achievementRewards).indexOf(thisId) > -1) {
    //check whether the acheivementRewards object has an entry for this id
    document.getElementById("achievementBoxInfo").innerHTML +=
      "<br><br><span style='color: #088; font-size: 16px; font-weight: bold; margin-right: 2px'>Reward gained: " +
      achievementRewards[thisId] +
      "</span>";
  }
...
async function achievementTabFlash() {
  ...
  while (game.currentTab !== 3) {
    ...
    let currentColourAmt =
      0.5 * Math.sin(Math.PI * 4 * (elapsedTime / 1000 - 0.125)) + 0.5; //this is a sin function that fluctuates between 0 and 1 with period of 0.5s
    achieveTab.style.background =
      "#bb" + lerpColour("bb", "ff", currentColourAmt) + "bb";
